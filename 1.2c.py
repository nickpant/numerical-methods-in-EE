...
# return a vector with the jth index of first nonzero element in row i
def bandwidth(A_passed):

    for i in range(len(A_passed), 0, -1):
...
def solve(A_passed, b_passed):
    ...
    print_matrix(A_passed)

    b = bandwidth(A_passed)
    ...
    for j in range(n):
        ...
        for i in range(j+1, min(n, j + 1 + b)):
            ...
            for k in range(j+1, i+1): # look ahead modification
